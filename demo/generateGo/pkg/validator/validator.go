...
// 部分方法的参数 可能来源于多种方式组合, 分开调用绑定 校验后组装
/*
	例：
	type EditIdDto struct { // param
		Id int64 `json:"id", binding:"required", comment:""`
	}
	type EditBodyDto struct { // body
		Id int64 `json:"id", binding:"required", comment:""`
	}
	...
	editId, err := ValidatParam[EditIdDto](c, binding.Query)
	body, err := ValidatParam[EditBodyDto](c, binding.JSON)
...
// 绑定并验证参数
func ValidatParam[T any](c *gin.Context, b binding.Binding) (*T, string, error) {
